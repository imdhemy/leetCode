...
class SlidingWindowTest {
    ...
    void slidingWindowMaxSum() {
        ...
        int[] input = new int[]{100, 200, 300, 400};
        int output = 700;
        Assertions.assertEquals(output, obj.slidingWindowMaxSum(input, 2));

        input = new int[]{1, 4, 2, 10, 23, 3, 1, 0, 20};
        output = 39;
        Assertions.assertEquals(output, obj.slidingWindowMaxSum(input, 4));

        input = new int[]{2, 3};
        output = -1;
        Assertions.assertEquals(output, obj.slidingWindowMaxSum(input, 3));
    }
}
